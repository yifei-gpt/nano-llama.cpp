// kv_cache.cpp — allocate per-layer F16 K/V buffers on a backend
...
void KvCache::init(int n_layer_, int n_embd_kv_, int n_slots_, int n_ctx, ggml_backend_buffer_type_t buft) {
    n_layer   = n_layer_;
    n_embd_kv = n_embd_kv_;
    n_slots   = n_slots_;
    ...
    NANO_LOG("kv-cache: %d layers, %d slots x %d cells = %d, %.1f MiB", n_layer, n_slots, n_ctx_pad, n_cells, mb);
}
