...
struct ModelRunner {
    ...
    // a batch's tokens/positions/KV targets; embd and mrope are set only for VLM image prefill
    struct Batch {
        std::vector<int32_t> token;
        std::vector<float>   embd;         // precomputed embeddings; overrides the token-id lookup
        std::vector<int32_t> pos;          // sequence position per token (for the causal mask)
        std::vector<int32_t> mrope;        // explicit M-RoPE positions; if empty, derived from pos
    ...
    // single-sequence multimodal decode: precomputed embeddings + explicit M-RoPE; logits for the last token
    ...
    int  scratch_cell() const { return kv.n_slots * kv.n_ctx_pad; }
    bool is_recurrent() const { return rc.active(); }
